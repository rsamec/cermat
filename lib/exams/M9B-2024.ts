...
const form = rootGroup({
  ...
  1: number(1.5, { prefix:'o',suffix: 'km' }),
  2: number(40, { prefix:'o', suffix: '%' }, twoPoints),
  3: group({
    3.1: mathExpr("-(1+1/9)", { hintType: 'fraction' }, twoPoints),
    3.2: mathExpr("-(7+7/9)", { hintType: 'fraction' }, twoPoints),
  }),
  4: group({
  ...
  7: group({
    7.1: mathExpr('x+x/2', { hintType: 'expression' }, twoPoints),
    7.2: number(52, { suffix:'modelů'}, twoPoints)
  }),
  8: group({
    8.1: number(64, { suffix: 'kruhů' }, twoPoints),
    8.2: number(20, { suffix: '.obrazec' }, twoPoints),
  }),
  9: selfEvaluateImage("9-result.jpeg", threePoints),
