...
const Timeline: React.FC<{ items: { text: string, publishedAt: string ,description?: string, slug: string }[] }> = ({ items }) => {
  return (
        ...
        {items.map((data, i, arr) =>
          ...
          <div key={data.slug} className="relative pl-8 sm:pl-32 py-6 group">
            ...
            <div className={cls([
              ...
              <time className="sm:absolute left-0 translate-y-0.5 inline-flex items-center justify-center text-xs font-semibold uppercase w-20 h-6 mb-3 sm:mb-0 text-white bg-blue-600 rounded-full"><DateFormatter dateString={data.publishedAt} format="MMM, yyyy" /></time>
